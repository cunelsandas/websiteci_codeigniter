...
class Banner extends CI_Controller
{
    ...
    public function index($name_type)
    {
        ...
        $data['site'] = site_url('banner/detail/' . $name_type);
        $data['site_add'] = site_url('banner/view/' . $name_type);
        $data['banner_position'] = $this->db->get('tb_banner_position')->result_array();
    ...
    public function view($name_type, $id = null)
    {
        ...
        $data['head'] = $head;
        $data['site'] = site_url('banner/save/' . $name_type . '/' . $id);
        ...
        if (isset($_POST['data'])) {
            ...
            if (isset($_POST['file_type'])) {
                $data['data'] = $this->db->select('*')
                    ->where('position', $_POST['file_type'])
                    ->like('name', $Search)
                    ->limit($limil, $offset)
                    ->get($table_name)
                    ->result_array();
            ...
            $data['site'] = site_url('banner/detail/' . $name_type . '/');
            $data['site_view'] = site_url('banner/view/' . $name_type . '/');
            $data['site_change'] = site_url('banner/change/' . $name_type . '/' . $page . '/');
            $data['site_remove'] = site_url('banner/remove_all/' . $name_type . '/' . $page . '/');
    ...
    public function save($name_type, $id = null)
    {
        ...
        $site_return = site_url('banner/view/' . $name_type . '/' . $masterid);
        if ($result) {
