...
class Path extends CI_Controller
{
    ...
    public function view($name_type, $id = null)
    {
        ...
        if ($id == null):
            ...
            $set_data = $this->db->where('view_id', $id)->get($set['table_name'])->row_array();
    ...
    public function save($name_type, $id = null)
    {
        ...
        if ($id) {
            ...
            $masterid = $this->db->order_by('view_id', 'DESC')->get($TableName)->row('view_id');
        }
        $site_return = site_url($ci_site . 'view/' . $name_type . '/' . $masterid);
        if ($result) {
            echo '<script>alert(\'บันทึกสำเร็จ\');window.location.href = \'' . site_url('paths/' . $name_type) . '\'</script>';
        } else {
    ...
    public function remove_all($name_type, $page, $id)
    {
        ...
        foreach ($files as $fKey => $fVal) {
            ...
            if (file_exists($file[$fKey])) {
                unlink($file[$fKey]);
                $Saveimg_model->Delete('filename', ['view_id' => $fVal['id']]);
            };
        }
        $result = $Saveimg_model->Delete($table_name, ['view_id' => $id]);
        $this->detail($name_type, $page);
    }
}