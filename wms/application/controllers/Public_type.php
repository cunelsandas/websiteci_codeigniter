...
class Public_type extends CI_Controller
{
    public $ci_name;
    ...
    public function view($name_type, $id = null)
    {
        ...
        if ($id == null):
            ...
            $set_data = $this->db->where('pubice_type_id', $id)->get($set['table_name'])->row_array();
    ...
    public function save($name_type, $id = null)
    {
        ...
        if ($result) {
            echo '<script>alert(\'บันทึกสำเร็จ\');window.location.href = \'' . site_url('public_types/' . $name_type) . '\'</script>';
        } else {
    ...
    public function remove_all($name_type, $page, $id)
    {
        ...
        foreach ($files as $fKey => $fVal) {
            ...
            if (file_exists($file[$fKey])) {
                unlink($file[$fKey]);
                $Saveimg_model->Delete('filename', ['pubice_type_id' => $fVal['id']]);
            };
        }
        $result = $Saveimg_model->Delete($table_name, ['pubice_type_id' => $id]);
        $this->detail($name_type, $page);
    }
}