...
class Group_menu extends CI_Controller
{
    public function index($name_type)
    {
        ...
        $data['site'] = site_url('group_menu/detail/' . $name_type);
        $data['site_add'] = site_url('group_menu/view/' . $name_type);
    ...
    public function view($name_type, $id = null)
    {
        ...
        if ($id == null):
            ...
            $set_data = $this->db->where('group_id', $id)->get($set['table_name'])->row_array();
            $files = $this->db->where('masterid', $id)->where('tablename', $table_name)->get($table_file_name)->result_array();
        endif;
        $data['head'] = $head;
        $data['site'] = site_url('group_menu/save/' . $name_type . '/' . $id);
    ...
    public function detail($name_type, $page = 1)
    {
        ...
        if (isset($_POST['data'])) {
            ...
            $data['site'] = site_url('group_menu/detail/' . $name_type . '/');
            $data['site_view'] = site_url('group_menu/view/' . $name_type . '/');
            $data['site_change'] = site_url('group_menu/change/' . $name_type . '/' . $page . '/');
            $data['site_remove'] = site_url('group_menu/remove_all/' . $name_type . '/' . $page . '/');
    ...
    public function save($name_type, $id = null)
    {
        ...
        if ($id) {
            ...
            $masterid = $this->db->order_by('group_id', 'DESC')->get($TableName)->row('group_id');
        }
        $site_return = site_url('group_menu/view/' . $name_type . '/' . $masterid);
        if ($result) {
            echo '<script>alert(\'บันทึกสำเร็จ\');window.location.href = \'' . site_url('group_menus/' . $name_type) . '\'</script>';
        } else {
    ...
    public function remove_all($name_type, $page, $id)
    {
        ...
        foreach ($files as $fKey => $fVal) {
            ...
            if (file_exists($file[$fKey])) {
                unlink($file[$fKey]);
                $Saveimg_model->Delete('filename', ['group_id' => $fVal['id']]);
            };
        }
        $result = $Saveimg_model->Delete($table_name, ['group_id' => $id]);
        $this->detail($name_type, $page);
    }
}