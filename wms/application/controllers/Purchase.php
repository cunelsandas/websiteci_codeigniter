...
class Purchase extends CI_Controller
{
    ...
    public function index($name_type)
    {
        ...
        $data['site_add'] = site_url('purchase/view/' . $name_type);
        $data['btn_add'] = 'เพิ่ม' . $set['sub_name'];
        $data['type'] = $this->db->get('tb_purchase_group')->result_array();
    ...
    public function view($name_type, $id = null)
    {
        ...
        $files = [];
        $theme = new theme();
        $view = 'config_purchase/view';
        $set = get_view($name_type);
        $table_name = $set['table_name'];
        $folder_name = $set['folder_name'];
        $table_file_name = 'filename';

        $file_type = $this->db->get('tb_purchase_group')->result_array();
        ...
        $data['head'] = $head;
        $data['site'] = site_url('purchase/save/' . $name_type . '/' . $id);
    ...
    public function detail($name_type, $page = 1)
    {
        ...
        if (isset($_POST['data'])) {
            ...
            $data['site_change'] = site_url('purchase/change/' . $name_type . '/' . $page . '/');
            $data['site_remove'] = site_url('purchase/remove_all/' . $name_type . '/' . $page . '/');
            $data['table_name'] = $table_name;
            $data['folder_name'] = $set['folder_name'];
            $data['pageNow'] = $page;
            $data['offset'] = $offset;
            $view = 'modules/config_purchase/detail';
    ...
    public function save($name_type, $id = null)
    {
        ...
        $site_return = site_url('purchase/view/' . $name_type . '/' . $masterid);
        if ($result) {
            echo '<script>alert(\'บันทึกสำเร็จ\');window.location.href = \'' . site_url('purchases/purchase') . '\'</script>';
        } else {
