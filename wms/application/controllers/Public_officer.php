...
class Public_officer extends CI_Controller
{
    public function index($name_type)
    {
        ...
        $data['site'] = site_url('public/detail/' . $name_type);
        $data['site_add'] = site_url('public/view/' . $name_type);
        $data['file_type'] = $this->db->get('set_public_type')->result_array();
    ...
    public function view($name_type, $id = null)
    {
        ...
        if ($id == null):
            ...
            $officer_subworkgroup = $this->db->where('workgroupid', $set_data['workgroupid'])->get('tb_public_subworkgroup')->result_array();
        endif;

        $data['head'] = $head;
        $data['site'] = site_url('public/save/' . $name_type . '/' . $id);
    ...
    public function save($name_type, $id = null)
    {
        ...
        $site_return = site_url('public/view/' . $name_type . '/' . $masterid);
        if ($result) {
    ...
    public function detail($name_type, $page = 1)
    {
        ...
        if (isset($_POST['data'])) {
            ...
            $data['site'] = site_url('public/detail/' . $name_type . '/');
            $data['site_view'] = site_url('public/view/' . $name_type . '/');
            $data['site_change'] = site_url('public/change/' . $name_type . '/' . $page . '/');
            $data['site_remove'] = site_url('public/remove_all/' . $name_type . '/' . $page . '/');
            $data['table_name'] = $table_name;
            $data['folder_name'] = $set['folder_name'];
            $data['type'] = $type;
            $data['head'] = $this->db->select('public_type_name')->where('public_type_id', $type)->get('set_public_type')->row('public_type_name');
            $view = 'modules/' . $view;
