...
class Workgroup extends CI_Controller
{
    public function index($name_type)
    {
        ...
        $data['site'] = site_url('workgroup/detail/' . $name_type);
        $data['site_add'] = site_url('workgroup/view/' . $name_type);
        $data['office_type'] = $this->db->get('set_office_type')->result_array();
        if ($name_type == 'subgroup') {
            $data['work_type'] = $this->db->get('tb_officer_workgroup')->result_array();
        }
    ...
    public function view($name_type, $id = null)
    {
        ...
        $data['head'] = $head;
        $data['site'] = site_url('workgroup/save/' . $name_type . '/' . $id);
    ...
    public function detail($name_type, $page = 1)
    {
        ...
        if (isset($_POST['data'])) {
            $table_name = $set['table_name'];
            $data['site'] = site_url('workgroup/detail/' . $name_type . '/');
            $data['site_view'] = site_url('workgroup/view/' . $name_type . '/');
            $data['site_change'] = site_url('workgroup/change/' . $name_type . '/' . $page . '/');
            $data['site_remove'] = site_url('workgroup/remove_all/' . $name_type . '/' . $page . '/');
            ...
            $data['pageNow'] = $page;
            $data['offset'] = $offset;
            $view = 'modules/config_workgroup/detail';
    ...
    public function save($name_type, $id = null)
    {
        ...
        $site_return = site_url('workgroup/view/' . $name_type . '/' . $masterid);
        if ($result) {
