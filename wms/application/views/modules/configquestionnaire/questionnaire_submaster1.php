<?php
	...
	if(!empty($id)){
		$sql="select * from tb_questionnaire_submaster where id=? and project_id=?";
	...
	if(isset($_POST['btsave'])){
		...
		switch($_POST['btsave']){
			case "add":
				$sql="insert into tb_questionnaire_submaster(project_id,name,type_id,master_id)values($p_id,'$name',$type_id,$master_id)";
				$msg="เพิ่มหัวข้อหลัก สำเร็จ";
				break;
			case "edit":
				$sql="update tb_questionnaire_submaster set name='$name',type_id='$type_id',master_id='$master_id' where id=$submaster_id";
...
<div class="card card-shadow" id="CardDetail">
    <div class="card-header">
        <div class="row">
            <div class="col-lg-12">
                <h4><?php $head = "หัวข้อรอง (Sub-Master)";
                    echo $head; ?></h4>
            </div>
        </div>
    </div>
