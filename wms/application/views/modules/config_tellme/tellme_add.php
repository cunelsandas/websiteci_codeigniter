...
if(isset($_POST['btwb'])){
	...
	if($chkver<>$_POST['txtver']){
		...
		$sql="INSERT INTO tb_help_maps(subject,detail,postby,datepost,ip,status) Values('".$_POST['txtsubject']."','".$_POST['detail1']."','".$_SESSION['name']."','".$_POST['dateInput']."','".$_SERVER['REMOTE_ADDR']."','0')";
		...
		$updatetran=UpdateTrans('tb_help_maps','add',$_SESSION['username'],'ID:'.$_POST['txtsubject']);

			echo"<script>alert('บันทึกข้อมูลเรียบร้อย');window.location.href='helpme?_modid=".$modid."&_mod=".$_GET['_mod']."';</script>";
