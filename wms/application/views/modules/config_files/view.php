    ...
    <form action="<?php echo $site; ?>" method="post" enctype="multipart/form-data">
        <div class="card-body">
            ...
            <div class="row mb-2">
                ...
                <div class="col-lg-5">
                    <select name="filetypeid" id="filetypeid" class="form-control select">
                        <option value="" selected disabled>ประเภทไฟล์</option>
                        <?php foreach ($file_type as $ftKey => $ftVal): ?>
                            <option value="<?php echo $ftVal['file_type_id'] ?>" <?php echo IsSelect($set_data['filetypeid'], $ftVal['file_type_id']) ?>><?php echo $ftVal['file_type_name'] ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
