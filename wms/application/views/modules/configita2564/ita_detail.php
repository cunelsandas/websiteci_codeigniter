<?php
	...
	if($rsShow || msyqli_num_rows($rsShow)>0){
		while($data=mysqli_fetch_assoc($rsShow)){
			$id=$data['id'];
			$menuid=$data['menuid'];
			$menuname=FindRS("select * from tb_ita_menu_2564 where id=".$menuid,"name");
			$submenuid=$data['submenuid'];
			$submenuname=FindRS("select * from tb_ita_submenu_2564 where id=".$submenuid,"name");
			$name=$data['name'];
			$listno=$data['listno'];
			...
			<table width='100%'>
				...
				<tr>
					...
					<td>
							<select name='submenu'>
								<?php
									$sql="select * from tb_ita_submenu_2564 order by menuid ASC ,listno ASC";
									$rsMenu=rsQuery($sql);
									if($rsMenu){
										while($dMenu=mysqli_fetch_assoc($rsMenu)){
											$id=$dMenu['id'];
