  ...
  <?php
	...
	$modname=FindRS("select modname from tb_mod where modid=$modid","modname");
		...
		$updatetran=UpdateTrans('$tablename','delete',$_SESSION['username'],'ID:'.$id);
			...
			echo"<script>window.location.href='accident_type?_modid=".$modid."&_mod=".$_GET['_mod']."';</script>";
		...
		if(isset($_POST['btsave'])){
			...
			if($rsupdate){
				...
				echo"<script>alert('บันทึกข้อมูลเรียบร้อย');window.location.href='accident_type?_modid=".$modid."&_mod=".$_GET['_mod']."';</script>";
					...
					if($rs){
						...
						while($data=mysqli_fetch_assoc($rs)){
							...
							$name=$data['name'];
								...
								echo "<tr><td>$name</td><td><a class=\"btn btn-warning btn-sm text-white\" href=\"accident_type?_modid=".$modid."&_mod=".$_GET['_mod']."&type=type&id=".$id."\"><i class=\"fa fa-edit btn-add\"></i></a>&nbsp;&nbsp;&nbsp;
                            <a class=\"btn btn-danger btn-sm text-white\" href=\"accident_type?_modid=".$modid."&_mod=".$_GET['_mod']."&type=type&del=".$id."\" onclick=\"return confirm('คุณต้องการลบรายการนี้ใช่หรือไม่?');\"><i class=\"fa fa-trash-o btn-remove\"></i></a></td></tr>";
