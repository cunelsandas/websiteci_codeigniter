<?php
	...
	if($rsShow || msyqli_num_rows($rsShow)>0){
		while($data=mysqli_fetch_assoc($rsShow)){
			$id=$data['id'];
			$menuid=$data['menuid'];
			$menuname=FindRS("select * from tb_ita_menu where id=".$menuid,"name");
			$submenuid=$data['submenuid'];
			$submenuname=FindRS("select * from tb_ita_submenu where id=".$submenuid,"name");
			$name=$data['name'];
			$listno=$data['listno'];
			$modname=$data['modname'];
			$link=$data['link'];
			$strTableDetail .="<tr><td style='padding: 10px;border-bottom: solid 1px black'>$menuname</td><td style='border-bottom: solid 1px black'>$submenuname</td><td style='border-bottom: solid 1px black'>$name</td><td style='border-bottom: solid 1px black;background-color: orange' align='center'>$listno</td><td style='border-bottom: solid 1px black;padding-left: 3px'>$modname</td><td style='border-bottom: solid 1px black;color: red;border-right: solid 1px black;border-left: solid 1px black;padding-left: 3px'>$link</td><td style='border-bottom: solid 1px black' align='center'><a class=\"btn btn-warning btn-sm text-white\" href=\"ita?_modid=".$modid."&_mod=".$_GET['_mod']."&type=".$type."&id=".$id."\"><i class=\"fa fa-edit btn-add\"></i></a>&nbsp;&nbsp;&nbsp;
<a class=\"btn btn-danger btn-sm text-white\" href=\"ita?_modid=".$modid."&_mod=".$_GET['_mod']."&type=".$type."&del=".$id."\" onclick=\"return confirm('คุณต้องการลบรายการนี้ใช่หรือไม่ ?');\"><i class=\"fa fa-trash-o btn-remove\"></i></a></td>";
		}
	...
	if(isset($_POST['btsave'])){
		...
		if($_POST['btsave']=="addnew"){
			...
			$rs=rsQuery($sql);
			if($rs){
				echo "<script>alert('".$msg."');window.location.href='ita?_modid=".$modid."&_mod=".$mod."&type=".$type."';</script>";
			}
	...
	if(isset($_GET['del'])){
        ...
        if($rs){
            ...
            echo"<script>window.location.href='ita?_modid=".$modid."&_mod=".$_GET['_mod']."&type=".$type."';</script>";
        }
    }
...
<div class='content-box'>
    <p align="center" style="margin-top: 2%;text-decoration: underline;font-size: 1.3rem;font-weight: bold">รายละเอียด การประเมิน ITA</p>
	<hr>
			...
			<table width='100%'>
				<tr>
					...
					<td>
							<select name='menu'>
								<?php
									$sql="select * from tb_ita_menu order by listno ASC";
									$rsMenu=rsQuery($sql);
									if($rsMenu){
				...
				<tr>
					...
					<td>
							<select name='submenu'>
								<?php
									$sql="select * from tb_ita_submenu order by menuid ASC ,listno ASC";
									$rsMenu=rsQuery($sql);
									if($rsMenu){
										while($dMenu=mysqli_fetch_assoc($rsMenu)){
											$id=$dMenu['id'];
				...
				<tr>
					...
					<td>
                        ...
                        <select name='files' id='files' onchange="getLink(this.value,'files');">
                            <option value=''>ไม่เลือก</option>
                            <?php
                            $sql="select * from set_file_type order by file_type_id ASC";
                            $rsfile=rsQuery($sql);
                            if($rsfile){
                                while($dfile=mysqli_fetch_assoc($rsfile)){
                                    $fid=($dfile['file_type_id']);
                                    $fname=$dfile['file_type_name'];
		...
		<div class='content-table'>
			<table width='100%'>
				<tr style="text-align: center;background-color: orange;height: 40px;">
					<th width='14%'>หัวข้อหลัก</th>
					<th width='14%'>หัวข้อย่อย
