    ...
    <div class="col-lg-12">
        <div class="card card-shadow">
            ...
            <div class="card-body">
                <div class="row mb-2">
                    <div class="col-lg-9">
                        <input type="text" name="search-data-detail" id="search-data-detail" class="form-control">
                    </div>
                    <div class="col-lg-3">
                        <select name="file_type" id="file_type" class="form-control">
                            <?php foreach ($file_type as $ftKey => $ftVal): ?>
                                <option value="<?php echo $ftVal['office_type_id'] ?>"><?php echo $ftVal['office_type_name']; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
...
<script>
    ...
    var page = 1;
    load_page({data: $search.val(), page: 1, file_type: $file_type.val()}, url);
    ...
    div.on('click', $remove, function (e) {
        ...
        if ($d) {
            $data = {data: '', page: page, file_type: $file_type.val()};
            url = $(this).attr('data-href');
            load_page($data, url);
        }
    });
