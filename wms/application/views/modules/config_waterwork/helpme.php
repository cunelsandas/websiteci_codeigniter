...
if ($type == "addnews") {             //ตรวจสอบก่อนว่ามีการตั้งค่าของ $_GET['type'] เป็นการเพิ่มข่าวใหม่หรือเปล่า
    ...
    if(isset($_GET['status'])){
        ...
        if($rs){
            ...
            echo"<script>window.location.href='waterwork?_modid=".$modid."&_mod=".$_GET['_mod']."';</script>";
        ...
        <div class="card-header">
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    ...
                    <h4><?php $head ="แจ้งซ่อมประปาสาธารณะ";
            ...
            if ($rs->num_rows > 0) {
                ...
                while ($row = $rs->fetch_assoc()) {
                    ...
                    if($row['status']=="0"){
                        ...
                        echo"<a class=\"btn btn-danger btn-sm text-white\" href=\"waterwork?_modid=".$modid."&_mod=".$_GET['_mod']."&status=1&no=".$row['id']."\"><i class=\"fa fa-thumbs-o-down\"></i></a>";

                    }else{

                        echo"<a class=\"btn btn-primary btn-sm text-white\" href=\"waterwork?_modid=".$modid."&_mod=".$_GET['_mod']."&status=0&no=".$row['id']."\"><i class=\"fa fa-thumbs-o-up\"></i></a>";

                    }

                    echo "</td>";

                      echo "<td width='100px' align=\"center\"><a class=\"btn btn-warning btn-sm text-white\" href=\"waterwork?_modid=" . $modid . "&_mod=" . $_GET['_mod'] . "&type=view&no=" . $row['id'] . "\">
<i class=\"fa fa-edit btn-add\"></i>
</a>&nbsp;&nbsp;&nbsp;&nbsp;<a class=\"btn btn-danger btn-sm text-white\" href=\"waterwork?_modid=" . $modid . "&_mod=" . $_GET['_mod'] . "&del=" . $row['id'] . "\" onclick=\"return confirm('คุณต้องการลบคำร้องนี้หรือไม่?');\">
