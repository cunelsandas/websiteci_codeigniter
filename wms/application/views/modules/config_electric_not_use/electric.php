...
<tr>
	...
	<td style="padding: 10px" width="330">&nbsp;รายการ</td>
...
$Query = rsQuery($sql); //คิวรี่คำสั่ง
	...
	if($totalp==0){
		...
		while($arr = mysqli_fetch_assoc($Query)){
			...
			echo"<td style='padding: 10px;margin-bottom: 3px'>&nbsp;".$arr['subject']."</td>";
			...
			if($arr['active']=="0"){
				...
				echo"<a class=\"btn btn-danger btn-sm text-white\" href=\"electric?_modid=".$modid."&_mod=".$_GET['_mod']."&active=1&no=".$arr['no']."\"><i class=\"fa fa-thumbs-o-down\"></i></a>";

			}else{

				echo"<a class=\"btn btn-primary btn-sm text-white\" href=\"electric?_modid=".$modid."&_mod=".$_GET['_mod']."&active=0&no=".$arr['no']."\"><i class=\"fa fa-thumbs-o-up\"></i></a>";

			}

			echo"</td>";

			 echo "<td width='100px' align=\"center\"><a class=\"btn btn-warning btn-sm text-white\" href=\"electirc?_modid=" . $modid . "&_mod=" . $_GET['_mod'] . "&type=view&no=" . $arr['no'] . "\">
<i class=\"fa fa-edit btn-add\"></i> 
</a> <a class=\"btn btn-danger btn-sm text-white\" href=\"electric?_modid=".$modid."&_mod=".$_GET['_mod']."&del=".$arr['no']."\" onclick=\"return confirm('คุณต้องการลบรายการนี้หรือไม่?');\"><i class=\"fa fa-trash-o btn-remove\"></i></a></td>";
	...
	if ($page > 1) {
		...
		echo "<a href=electric?_mod=".$_GET['_mod']."&_modid=".$modid."&page=1> << </a>&nbsp;&nbsp;";

		echo "<a href=electric?_mod=".$_GET['_mod']."&_modid=".$modid."&page=$back> < </a>&nbsp;&nbsp;";
	...
	For ($i=$start ; $i<=$end ; $i++) {
		...
		if ($i == $page ) {
			...
			echo "&nbsp;<a href=electric?_mod=".$_GET['_mod']."&_modid=".$modid."&page=".$i." style=\"color:$bgcolor\"><font color=$bgcolor>".$i."</font></a>&nbsp;&nbsp;&nbsp;" ;
	...
	if ($page < $totalpage) {
		...
		echo "&nbsp;&nbsp;<a href=electric?_mod=".$_GET['_mod']."&_modid=".$modid."&page=$next> > </a>";

		echo "&nbsp;<a href=electric?_mod=".$_GET['_mod']."&_modid=".$modid."&page=$totalpage> >> </a>";
