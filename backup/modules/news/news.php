...
$tablename="tb_news";  //ชื่อ ตารางข้อมูล
$picfolder="fileupload/news/";  // ชื่ีอโฟลเดอร์เก็บภาพ
...
if($no<>""){
	include"modules/news/news_view.php";
}else{
$pagelen = 15; //จำนวนที่แสดงผลข้อมูลต่อหน้า
	...
	$sql = "select * from $tablename Where status='1' order by datepost DESC, no DESC Limit $goto,$pagelen"; //ทำการแสดงผลโดยใช้คำสั่ง Limit เพื่อแสดงจำนวนข้อมูลต่อหน้า
...
//	$totalp = mysqli_num_rows($Query); // หาจำนวน record ที่เรียกออกมา
	if($totalrecords==0){
		...
		while($arr = mysqli_fetch_array($Query)){
			...
			echo"<div class=\"subject\"><a href=\"index.php?_mod=".encode64($mod)."&no=".encode64($arr['no'])."\">".$arr['subject']."</a></div>";
