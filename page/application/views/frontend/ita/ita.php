...
<div class='body-bg_ita'><p class='header_ita'>แบบประเมิน ITA (Integrity and Transparency Assessment)</p><p class='header_ita'><?php echo WEBNAME?></p><br>
<?php
...
function rsQuery($sql){
	...
	if($rs){
		...
		while($data=mysqli_fetch_assoc($rs)){
			...
			$sql_sub="select * from tb_ita_submenu where menuid=$id Order by listno ASC";
				...
				if($rsSub){
					...
					while($dataSub=mysqli_fetch_assoc($rsSub)){
						...
						$sql_detail="select * from tb_ita_detail where submenuid=$submenuid Order by listno ASC";
