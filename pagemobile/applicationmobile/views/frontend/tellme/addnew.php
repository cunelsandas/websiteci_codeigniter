...
//                    echo "<script>alert('The file exists')</script>";
            ...
            $sql = "INSERT INTO tb_tellme(id,subject,detail,detail2,result,lat,lng,postby,address,email,datepost,ip,typewb,status,updatetime,image_key) Values('','" . $_POST['f_subject'] . "','" . $_POST['f_detail'] . "','" . $_POST['f_detail2'] . "','','" . $_POST['f_lat'] . "','" . $_POST['f_lng'] . "','" . $_POST['f_name'] . "','" . $_POST['f_address'] . "','" . $_POST['f_email'] . "','" . $_POST['f_dateInput'] . "','" . $_SERVER['REMOTE_ADDR'] . "','1','1','0','" . $_POST['f_key'] . "')";
            $rs = rsQuery($sql);
            if ($rs) {
                echo "<script>alert('เรื่องของคุณได้ถูกส่งไปยังผุ้ที่เกี่ยวข้องแล้วค่ะ ระบบจะนำคุณไปที่หน้าแรก');window.location.href='../../page';</script>";
...
<?php
$sql = "select * from tb_tellme order by id desc LIMIT 1";
$rs = rsQuery($sql);
$rs->num_rows;
$row = $rs->fetch_assoc();
...
<hr style="background-color: #fcdee2;height: 10px;margin-top: -2%">
<h4 style="width: 100%;height: 35px; font-weight: bold; text-align: center;margin-left:;background-color: pink;color: white;margin-top: -1.3%;font-size: 1.5rem"ิ>แจ้งเรื่องร้องเรียนการทุจริต</h4>
...
<!--            </div>-->
            <div class="form-group" style="font-size: 20px">
                <div class="col-sm-10">
                    <label class="control-label"><b>ข้อมูลผู้ร้องเรียน</b></label>
                    <input style="font-size: 20px" type="text" class="form-control" id="f_subject" name="f_subject" required>
                </div>
            </div>

            <div class="form-group" style="font-size: 20px">
                <div class="col-sm-10">
                    <label class="control-label"><b>ข้อมูลผู้ถูกร้องเรียน (เจ้าหน้าที่ในสังกัดเทศบาลตำบลแม่แรม)</b></label>
                    <textarea style="font-size: 20px"  type="text" class="form-control" id="f_detail" name="f_detail" required></textarea>
                </div>
            </div>
            <div class="form-group" style="font-size: 20px">
                <div class="col-sm-10">
                    <label class="control-label"><b>ลักษณะพฤติกรรมตามการร้องเรียน</b></label>
                    <textarea style="font-size: 20px" type="text" class="form-control" id="f_detail2" name="f_detail2" required></textarea>
                </div>
            </div>
...
<!--    </div>-->
    ...
    <center>
        <button style="margin-bottom: 2%" type="submit" name="sub" class="btn btn-primary">บันทึก</button>
    </center>
    </form>
    </div>
